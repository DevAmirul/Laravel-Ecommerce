        ...
        <div class="col-12">
            <div class="card my-4">
                ...
                <div class="col-6 m-auto mt-3 ">
                    <form wire:submit.prevent='updateProduct' class="row g-3 shadow-lg rounded-3" enctype="multipart/form-data">
                        ...
                        <div class="form-floating col-md-12 shadow-lg">
                            <input wire:model.defer='regular_price' type="text" class="form-control" id="price" placeholder="200.00">
                            <label for="price">Regular Price</label>
                            @error('regular_price') <small>{{ $message }}</small> @enderror
                        </div>
                        <div class="form-floating col-md-12 shadow-lg">
                            <input wire:model.defer='sale_price' value="0" type="text" class="form-control" id="saleprice" placeholder="200.00">
                        ...
                        <div class="form-floating col-md-12 shadow-lg">
                            <input wire:model.defer='newImage' type="file" class="form-control" id="file">
                            <label for="file">Image uplode</label>
                            @if ($newImage)
                            <img src="{{ $newImage->temporaryUrl() }}" class="rounded" width="120" alt="">
                            @else
                            <img src="{{ asset('assets/images/products') }}/{{ $image }}" class="rounded" width="120" alt="">
                            @endif
                            @error('newImage') <small>{{ $message }}</small> @enderror
                        </div>
                        <div class="form-floating col-md-12 shadow-lg">
                            <input wire:model.defer='newGalleryImages' type="file" class="form-control" id="file" multiple>
                            <label for="file">Image uplode</label>
                            @if ($newGalleryImages)
                            @foreach ($newGalleryImages as $newImg)
                            <img src="{{ $newImg->temporaryUrl() }}" class="rounded" width="120" alt="">
                            @endforeach
                            @elseif ($galleryImages)
                            @foreach ($galleryImages as $img)
                            <img src="{{ asset('assets/images/products') }}/{{ $img }}" class="rounded" width="120" alt="">
                            @endforeach
                            @endif
                            @error('newGalleryImages') <small>{{ $message }}</small> @enderror
                        </div>
